shinyServer(function(input, output, session) {
  ...
  if (!"./saved-models" %in% list.dirs()) {
    dir.create("./saved-models")
  }
  
  # load the previously trained models - Note: you can delete files in the saved-models directory
  for (rdsfile in list.files(path = "saved-models", pattern = "\\.rds")) {
    name <- gsub(rdsfile, pattern = "\\.rds$", replacement = "")
    rdsfile <- paste0(".", .Platform$file.sep, "saved-models", .Platform$file.sep, rdsfile)
  ...
  # reactive getData ----
  getData <- reactive({
    d <- read.csv(file = "vacc_data.csv", row.names = "ID", stringsAsFactors = TRUE)
